...
Upload a log (or pick a sample) → watch the agent trace stream live →
read incidents, plans, Slack/JIRA outcomes and the synthesized cookbook.
"""
...
def run_suite(file_path: str | None):
    if not file_path:
    ...
    for chunk in graph.stream({"log_path": file_path}, stream_mode="values"):
        final_state = chunk
        trace = chunk.get("trace", [])
        if len(trace) > len(trace_lines):
            trace_lines = trace
        incidents_md = _render_incidents(chunk)
        yield "\n".join(trace_lines), incidents_md, chunk.get("cookbook", "")
...
def _render_incidents(state: dict) -> str:
    ...
    for inc in classification.incidents:
        ...
        out.append(f"`{inc.affected_component}` · ×{inc.occurrence_count} · first seen {inc.first_seen}")
        plan = plans.get(inc.incident_id)
        if plan:
            out.append(f"\n**Plan** ({plan.runbook_source or 'first principles'}):")
            for s in plan.steps:
                risk = f" ⚠ _{s.risk}_" if plan.escalate else ""
                out.append(f"{s.order}. {s.action}{risk}")
...
with gr.Blocks(title=f"Incident Suite v{__version__}") as demo:
    gr.Markdown(f"# 🚨 Incident Suite `v{__version__}`\nMulti-agent DevOps incident analysis — LangGraph orchestrated.")
    with gr.Row():
    ...
    run_btn.click(run_suite, inputs=[file_in], outputs=[trace_out, incidents_out, cookbook_out])
